...
URL. The approved Core digest is checked by the workflow and recorded in its
summary because the upstream feed schema itself is tag-based. The page must
never contain user, device, network, or installation data.
...
Core version tag must never be republished.
...
2. Publish unique ABEDOME Supervisor and Core development versions, verify
   their approved digests, and never republish those tags.
