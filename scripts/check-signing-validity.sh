...
too_soon()  { ! openssl x509 -in "$1" -noout -checkend "$((MIN_DAYS * 86400))" >/dev/null; }

if [ "$(openssl x509 -in "$CERT" -noout -subject_hash)" = \
  "$(openssl x509 -in "$CERT" -noout -issuer_hash)" ]; then
  openssl verify -check_ss_sig -CAfile "$CERT" -no-CApath "$CERT" >/dev/null 2>&1 \
    || fail "self-issued certificate is not a currently valid self-signed certificate"
else
  openssl verify -CAfile "$KEYRING" -no-CApath "$CERT" >/dev/null 2>&1 \
    || fail "signing certificate is not currently valid or does not chain to ${KEYRING}"
fi
...
cur="$CERT"; hops=0
while :; do
  if [ "$(openssl x509 -in "$cur" -noout -subject_hash)" = \
    "$(openssl x509 -in "$cur" -noout -issuer_hash)" ]; then
    openssl verify -check_ss_sig -no_check_time -CAfile "$cur" -no-CApath "$cur" \
      >/dev/null 2>&1 || fail "self-issued certificate is not self-signed"
    break
  fi
  ...
  if too_soon "$next"; then
    fail "CA certificate '$(subject "$next")' expires $(enddate "$next") (within ${MIN_DAYS} days). A new CA should be generated, the keyrings (buildroot-external/ota/*-ca.pem) updated, and the cross-signed intermediate added to the bundle."
  fi
...
if too_soon "$CERT"; then
  fail "Signing certificate expires $(enddate "$CERT") (within ${MIN_DAYS} days). A new leaf certificate needs to be generated and the CI signing key updated."
fi

echo "Signing certificate and CA chain (in total $((hops + 1)) certificates) are valid now and for at least ${MIN_DAYS} days."
